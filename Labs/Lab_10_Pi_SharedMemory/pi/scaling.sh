...
#================= Compilar el cpp ======================
echo "Compilando CalculoPi_SM.cpp..."
g++ CalculoPi_SM.cpp -o CalculoPi_SM.x -fopenmp

if [ $? -ne 0 ]; then
    echo "Error al compilar CalculoPi_SM.cpp"
    exit 1
fi
echo "Compilación exitosa: CalculoPi_SM.x generado"
...
for (( t=1; t<=${MAX_THREADS}; t++ )); do
    ...
    OMP_NUM_THREADS=$t ./CalculoPi_SM.x
done

#=================== Speedup =========================
T1=$(head -n 1 scaling.dat | awk '{print $2}')
awk -v T1="$T1" '{printf "%d %.6f\n", $1, T1/$2}' scaling.dat > speedupVector.dat
