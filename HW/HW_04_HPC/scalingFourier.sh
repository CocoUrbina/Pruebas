...
g++ fourier.cpp -o fourier.x -fopenmp

if [ $? -ne 0 ]; then
    echo "Error al compilar fourier.cpp"
    exit 1
fi
echo "Compilación exitosa: fourier.x generado"
...
for (( t=1; t<=${MAX_THREADS}; t++ )); do
    ...
    OMP_NUM_THREADS=$t ./fourier.x >> scaling.dat
done
...
# scaling.dat > speedupFourier.dat
# awk lee scaling.dat y la salida formateada se redirige a speedupFourier.dat.
